...
func getConfigManager(rp viper.RemoteProvider) (crypt.ConfigManager, error) {
	...
	if rp.SecretKeyring() != "" {
		...
		case "zookeeper":
			config := rp.Config()
			cm, err = crypt.NewZookeeperConfigManager([]string{rp.Endpoint()}, rp.Path(), config["user"], config["password"], kr)
		}
	} else {
		switch rp.Provider() {
		case "etcd":
		...
		case "zookeeper":
			config := rp.Config()
			cm, err = crypt.NewStandardZookeeperConfigManager([]string{rp.Endpoint()}, rp.Path(), config["user"], config["password"])
		}
	}
	if err != nil {
		return nil, err
